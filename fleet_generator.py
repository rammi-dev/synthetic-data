"""
Fleet-scale pump data generator.
Creates thousands of unique pump device time series as parquet files.

Usage:
    python fleet_generator.py --num-devices 6000 --duration-days 365 --output-dir /data/pump_fleet
    python fleet_generator.py --num-devices 10 --duration-days 30 --output-dir test_fleet  # quick test
"""
...
@dataclass
class DeviceConfig:
    device_id: str                  # "device_000000"
    seed: int
    duration_days: float
    failure_type: str | None        # None, 'bearing', 'impeller', 'radial'
    ...
    p_offset: float                 # pressure offset from nominal
...
class FleetConfig:
    ...
    base_seed: int = 12345
    output_dir: str = 'fleet_data'
    # Failure distribution
    healthy_fraction: float = 0.4
    bearing_fraction: float = 0.2
    impeller_fraction: float = 0.2
    radial_fraction: float = 0.2
...
class DeviceConfigSampler:
    ...
    ALL_DECOY_TYPES = ['highload_step', 'highload_ramp', 'bp_step', 'bp_ramp']
    ...
    def sample_all(self) -> list[DeviceConfig]:
        ...
        types = (
            ...
            + ['impeller'] * int(n * self.cfg.impeller_fraction)
            + ['radial'] * int(n * self.cfg.radial_fraction)
        )
        ...
        for i in range(n):
            ...
            # decoy_types: random subset of 1-4
            n_decoys = int(rng.integers(1, 5))  # 1..4
            ...
            # p_offset: normal(0, PDISCH_NOM * 0.03)
            po = float(rng.normal(0, PDISCH_NOM * 0.03))
...
_WORKER_DECOY_CACHE: dict | None = None


def _worker_init():
    """Called once per worker process to build shared caches."""
    global _WORKER_DECOY_CACHE

    # Build the template cache (populates _TEMPLATE_CACHE in sample_generator)
    _build_template()

    # Pre-simulate all 4 decoy templates
    _WORKER_DECOY_CACHE = {}
    save_freq_s = SAVE_FREQ
    change_row = CHANGE_AT_S // save_freq_s
    for name, load_fn in _DECOY_LOAD_FNS.items():
        ddf = run_progpy(load_fn, WEAR_X0_HEALTHY, faulty=False)
        decoy_raw = ddf[_SIGNAL_COLS].values[change_row:]
        _WORKER_DECOY_CACHE[name] = decoy_raw
...
def generate_fleet(fleet_cfg: FleetConfig):
    ...
    # 5. Update manifest
    # Read existing manifest, update completed entries
...
def parse_args() -> FleetConfig:
    parser = argparse.ArgumentParser(
        description="Fleet-scale pump data generator",
    ...
    parser.add_argument('--output-dir', type=str, default='fleet_data')
    parser.add_argument('--healthy-fraction', type=float, default=0.4)
    parser.add_argument('--bearing-fraction', type=float, default=0.2)
    parser.add_argument('--impeller-fraction', type=float, default=0.2)
    parser.add_argument('--radial-fraction', type=float, default=0.2)
    ...
    cfg = FleetConfig(
        ...
        impeller_fraction=args.impeller_fraction,
